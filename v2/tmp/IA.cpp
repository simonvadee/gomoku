...
int		IA::megaval(Pos& pos, PLAYER player)
{
  ...
  for (int x = 0; x < _size; ++x)
	...
	tmp.x = x;
	tmp.y = y;
	weightPlayer += _board->getAlignement(_map, tmp, _dir[HORIZONTAL], player, false) - 1;
	weightPlayer += _board->getAlignement(_map, tmp, _dir[VERTICAL], player, false) - 1;
	weightPlayer += _board->getAlignement(_map, tmp, _dir[DIAGONAL_RL], player, false) - 1;
	weightPlayer += _board->getAlignement(_map, tmp, _dir[DIAGONAL_LR], player, false) - 1;
	// weightPlayer = MAX(weightPlayer, _board->getAlignement(_map, tmp, _dir[VERTICAL], player, false) - 1);
	// weightPlayer = MAX(weightPlayer, _board->getAlignement(_map, tmp, _dir[DIAGONAL_LR], player, false) - 1);
	// weightPlayer = MAX(weightPlayer, _board->getAlignement(_map, tmp, _dir[DIAGONAL_RL], player, false) - 1);
      }
  // for (int x = 0; x < _size; ++x)
  //   for (int y = 0; y < _size; ++y)
  //     {
  ...
  // 	tmp.x = x;
  // 	tmp.y = y;
  // 	weightOpponent = MAX(weightOpponent, _board->getAlignement(_map, tmp, _dir[HORIZONTAL], static_cast<PLAYER>(OPPONENT(player)), false) - 1);
  // 	weightOpponent = MAX(weightOpponent, _board->getAlignement(_map, tmp, _dir[VERTICAL], static_cast<PLAYER>(OPPONENT(player)), false) - 1);
  // 	weightOpponent = MAX(weightOpponent, _board->getAlignement(_map, tmp, _dir[DIAGONAL_LR], static_cast<PLAYER>(OPPONENT(player)), false) - 1);
  // 	weightOpponent = MAX(weightOpponent, _board->getAlignement(_map, tmp, _dir[DIAGONAL_RL], static_cast<PLAYER>(OPPONENT(player)), false) - 1);
  //     }
  // if (pos.x == 2 && pos.y == 0)
  //   {
  //     std::cout << (int)_map[0][0] << " and " << (int)_map[1][0] << " = " << weight << std::endl;
  //   }
  _map[pos.x][pos.y] = 0;
  return weightPlayer;
}
